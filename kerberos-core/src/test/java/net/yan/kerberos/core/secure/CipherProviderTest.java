...
public class CipherProviderTest {
    ...
    private static final Logger log = LoggerFactory.getLogger(CipherProviderTest.class);
    ...
    public void testCrypto() throws GeneralSecurityException, IOException, ClassNotFoundException {
        SecretKey secretKey = cipherProvider.generateKey(key);
        assertNotNull(secretKey);

        KeySpec keySpec = new DESedeKeySpec(key.getBytes());
        SecretKey _secretKey = cipherProvider.generateKey(keySpec);
        ...
        String t1 = cipherProvider.encryptString(strToEncrypt, keySpec);
        assertNotNull(t1);
        assertTrue(t1.length() > 0);
        String t2 = cipherProvider.encryptString(strToEncrypt, secretKey);
        assertNotNull(t2);
        assertTrue(t2.length() > 0);

        assertEquals(t1, t2);

        String t3 = cipherProvider.decryptString(t1, keySpec);
        assertNotNull(t3);
        assertTrue(t3.length() > 0);
        assertEquals(strToEncrypt, t3);

        String t4 = cipherProvider.decryptString(t1, secretKey);
        assertNotNull(t4);
        assertTrue(t4.length() > 0);
        assertEquals(strToEncrypt, t4);

        String t5 = cipherProvider.decryptString(t2, keySpec);
        assertNotNull(t5);
        assertTrue(t5.length() > 0);
        assertEquals(strToEncrypt, t5);

        String t6 = cipherProvider.decryptString(t2, secretKey);
        assertNotNull(t6);
        ...
        String t7 = cipherProvider.encryptObject(o1, key);
        assertNotNull(t7);
        log.info(t7);
        Crypto o2 = cipherProvider.decryptObject(t7, key);
        assertEquals(o1, o2);
    }
